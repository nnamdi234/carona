...
from models import Base, User, Ride, Vehicle, RidePassenger
from utils.security import hash_password, verify_password


async def run_tests():
    print("--- 1. Testing Models Registration ---")
    assert "users" in Base.metadata.tables
    assert "vehicles" in Base.metadata.tables
    assert "rides" in Base.metadata.tables
    assert "ride_passengers" in Base.metadata.tables
    assert "routes" not in Base.metadata.tables
    print("Models metadata tables verified successfully!")

    print("\n--- 2. Testing Security Utility (hash_password & verify_password) ---")
    ...
    print("\n--- 3. Setting up In-Memory SQLite Test Database ---")
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        print("\n--- 4. Testing User Registration (/users/register) ---")
        user_payload = {
            "first_name": "Alice",
            "last_name": "Smith",
        ...
        print("\n--- 5. Testing User Login (/users/login) ---")
        # Successful login
        login_payload = {
            "email": "alice@example.com",
            "password": "strongPassword123"
        ...
        print("\n--- 6. Testing Soft-deleted Account Login ---")
        async with async_session() as session:
            deleted_user = User(
                first_name="Deleted",
                last_name="User",
